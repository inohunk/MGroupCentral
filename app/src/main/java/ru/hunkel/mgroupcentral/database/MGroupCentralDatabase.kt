...
abstract class MGroupCentralDatabase : RoomDatabase() {
    abstract fun trackingModel(): ModulesDao

    companion object {
        private var INSTANCE: MGroupCentralDatabase? = null

        fun getInstance(context: Context): MGroupCentralDatabase {
            if (INSTANCE == null) {
                runBlocking {
                    INSTANCE = Room.databaseBuilder(
                        context.applicationContext,
                        MGroupCentralDatabase::class.java,
                        "mgroup-database"
                    )
